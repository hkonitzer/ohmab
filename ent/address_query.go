...
type AddressQuery struct {
	...
	withTimetables      *TimetableQuery
	modifiers           []func(*sql.Selector)
	loadTotal           []func(context.Context, []*Address) error
	withNamedBusiness   map[string]*BusinessQuery
	withNamedTimetables map[string]*TimetableQuery
...
func (aq *AddressQuery) QueryBusiness() *BusinessQuery {
	query := (&BusinessClient{config: aq.config}).Query()
...
func (aq *AddressQuery) QueryTimetables() *TimetableQuery {
	query := (&TimetableClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AddressQuery) Clone() *AddressQuery {
	...
	return &AddressQuery{
		...
		withBusiness:   aq.withBusiness.Clone(),
		withTimetables: aq.withTimetables.Clone(),
...
func (aq *AddressQuery) WithBusiness(opts ...func(*BusinessQuery)) *AddressQuery {
	...
	aq.withBusiness = query
	return aq
}

// WithTimetables tells the query-builder to eager-load the nodes that are connected to
// the "timetables" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AddressQuery) WithTimetables(opts ...func(*TimetableQuery)) *AddressQuery {
	query := (&TimetableClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withTimetables = query
	return aq
}
...
func (aq *AddressQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Address, error) {
	...
	if query := aq.withBusiness; query != nil {
		if err := aq.loadBusiness(ctx, query, nodes,
			func(n *Address) { n.Edges.Business = []*Business{} },
			func(n *Address, e *Business) { n.Edges.Business = append(n.Edges.Business, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withTimetables; query != nil {
		if err := aq.loadTimetables(ctx, query, nodes,
			func(n *Address) { n.Edges.Timetables = []*Timetable{} },
			func(n *Address, e *Timetable) { n.Edges.Timetables = append(n.Edges.Timetables, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range aq.withNamedBusiness {
		if err := aq.loadBusiness(ctx, query, nodes,
			func(n *Address) { n.appendNamedBusiness(name) },
			func(n *Address, e *Business) { n.appendNamedBusiness(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range aq.withNamedTimetables {
		if err := aq.loadTimetables(ctx, query, nodes,
			func(n *Address) { n.appendNamedTimetables(name) },
			func(n *Address, e *Timetable) { n.appendNamedTimetables(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (aq *AddressQuery) loadBusiness(ctx context.Context, query *BusinessQuery, nodes []*Address, init func(*Address), assign func(*Address, *Business)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(address.BusinessTable)
		s.Join(joinT).On(s.C(business.FieldID), joinT.C(address.BusinessPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(address.BusinessPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(address.BusinessPrimaryKey[1]))
...
func (aq *AddressQuery) loadTimetables(ctx context.Context, query *TimetableQuery, nodes []*Address, init func(*Address), assign func(*Address, *Timetable)) error {
	...
	query.withFKs = true
	query.Where(predicate.Timetable(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(address.TimetablesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.address_timetables
		if fk == nil {
			return fmt.Errorf(`foreign-key "address_timetables" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "address_timetables" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedBusiness tells the query-builder to eager-load the nodes that are connected to the "business"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (aq *AddressQuery) WithNamedBusiness(name string, opts ...func(*BusinessQuery)) *AddressQuery {
	query := (&BusinessClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if aq.withNamedBusiness == nil {
		aq.withNamedBusiness = make(map[string]*BusinessQuery)
	}
	aq.withNamedBusiness[name] = query
	return aq
}

// WithNamedTimetables tells the query-builder to eager-load the nodes that are connected to the "timetables"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (aq *AddressQuery) WithNamedTimetables(name string, opts ...func(*TimetableQuery)) *AddressQuery {
	...
	if aq.withNamedTimetables == nil {
		aq.withNamedTimetables = make(map[string]*TimetableQuery)
	}
	aq.withNamedTimetables[name] = query
	return aq
}
