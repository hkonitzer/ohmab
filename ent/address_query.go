...
func (aq *AddressQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Address, error) {
	...
	if query := aq.withBusiness; query != nil {
		if err := aq.loadBusiness(ctx, query, nodes, nil,
			func(n *Address, e *Business) { n.Edges.Business = e }); err != nil {
			return nil, err
		}
	}
...
func (aq *AddressQuery) loadBusiness(ctx context.Context, query *BusinessQuery, nodes []*Address, init func(*Address), assign func(*Address, *Business)) error {
	...
	for i := range nodes {
		if nodes[i].business_addresses == nil {
			continue
		}
		fk := *nodes[i].business_addresses
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "business_addresses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
