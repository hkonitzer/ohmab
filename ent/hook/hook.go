...
// The TimetableFunc type is an adapter to allow the use of ordinary
// function as Timetable mutator.
type TimetableFunc func(context.Context, *ent.TimetableMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TimetableFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TimetableMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TimetableMutation", m)
}
