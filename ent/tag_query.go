...
func (tq *TagQuery) QueryBusiness() *BusinessQuery {
	query := (&BusinessClient{config: tq.config}).Query()
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withBusiness: tq.withBusiness.Clone(),
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withBusiness; query != nil {
		if err := tq.loadBusiness(ctx, query, nodes,
			func(n *Tag) { n.Edges.Business = []*Business{} },
			func(n *Tag, e *Business) { n.Edges.Business = append(n.Edges.Business, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range tq.withNamedBusiness {
		if err := tq.loadBusiness(ctx, query, nodes,
			func(n *Tag) { n.appendNamedBusiness(name) },
			func(n *Tag, e *Business) { n.appendNamedBusiness(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedUser {
		if err := tq.loadUser(ctx, query, nodes,
			func(n *Tag) { n.appendNamedUser(name) },
			func(n *Tag, e *User) { n.appendNamedUser(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TagQuery) loadBusiness(ctx context.Context, query *BusinessQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Business)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.BusinessTable)
		s.Join(joinT).On(s.C(business.FieldID), joinT.C(tag.BusinessPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.BusinessPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.BusinessPrimaryKey[1]))
...
func (tq *TagQuery) WithNamedBusiness(name string, opts ...func(*BusinessQuery)) *TagQuery {
	...
	if tq.withNamedBusiness == nil {
		tq.withNamedBusiness = make(map[string]*BusinessQuery)
	}
	tq.withNamedBusiness[name] = query
	return tq
}
