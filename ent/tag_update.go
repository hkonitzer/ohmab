...
func (tu *TagUpdate) RemoveBusinesIDs(ids ...uuid.UUID) *TagUpdate {
	tu.mutation.RemoveBusinesIDs(ids...)
	return tu
}
...
func (tu *TagUpdate) RemoveBusiness(b ...*Business) *TagUpdate {
	...
	return tu.RemoveBusinesIDs(ids...)
}
...
func (tu *TagUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := tu.mutation.Comment(); ok {
		_spec.SetField(tag.FieldComment, field.TypeString, value)
	}
	if tu.mutation.CommentCleared() {
		_spec.ClearField(tag.FieldComment, field.TypeString)
	}
	if tu.mutation.BusinessCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.BusinessTable,
			Columns: tag.BusinessPrimaryKey,
			Bidi:    false,
	...
	if nodes := tu.mutation.RemovedBusinessIDs(); len(nodes) > 0 && !tu.mutation.BusinessCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.BusinessTable,
	...
	if nodes := tu.mutation.BusinessIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.BusinessTable,
...
func (tuo *TagUpdateOne) AddBusinesIDs(ids ...uuid.UUID) *TagUpdateOne {
	tuo.mutation.AddBusinesIDs(ids...)
	return tuo
}
...
func (tuo *TagUpdateOne) RemoveBusiness(b ...*Business) *TagUpdateOne {
	...
	return tuo.RemoveBusinesIDs(ids...)
}
...
func (tuo *TagUpdateOne) sqlSave(ctx context.Context) (_node *Tag, err error) {
	...
	if tuo.mutation.BusinessCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.BusinessTable,
	...
	if nodes := tuo.mutation.RemovedBusinessIDs(); len(nodes) > 0 && !tuo.mutation.BusinessCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.BusinessTable,
	...
	if nodes := tuo.mutation.BusinessIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.BusinessTable,
