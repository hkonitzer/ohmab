...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withBusinesses; query != nil {
		if err := uq.loadBusinesses(ctx, query, nodes,
			func(n *User) { n.Edges.Businesses = []*Business{} },
			func(n *User, e *Business) { n.Edges.Businesses = append(n.Edges.Businesses, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range uq.withNamedBusinesses {
		if err := uq.loadBusinesses(ctx, query, nodes,
			...
			func(n *User, e *Business) { n.appendNamedBusinesses(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range uq.withNamedTags {
		if err := uq.loadTags(ctx, query, nodes,
...
func (uq *UserQuery) loadBusinesses(ctx context.Context, query *BusinessQuery, nodes []*User, init func(*User), assign func(*User, *Business)) error {
	...
	query.Where(predicate.Business(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BusinessesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_businesses
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_businesses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_businesses" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) WithNamedBusinesses(name string, opts ...func(*BusinessQuery)) *UserQuery {
	...
	if uq.withNamedBusinesses == nil {
		uq.withNamedBusinesses = make(map[string]*BusinessQuery)
	}
	uq.withNamedBusinesses[name] = query
	return uq
}
